...
# ╔═╡ 60480bbe-98e4-45b8-8b56-558b2f4ebfed
md"""
# Setup Design

Tipp: Don't use too many subjects/items/trials, it will just take longer ;)
"""
...
| | |
|---|---|
|N-Subjects |$(@bind nsub PlutoUI.Slider(1:5:50;default=15,show_value=true) )|
|N-Items |$(@bind nitem PlutoUI.Slider(1:5:50;default=15,show_value=true) )|
|Repeat Trials |$(@bind repeats PlutoUI.Slider(1:1:30;default=20,show_value=true) )|
|How many levels in condition |$(@bind condlevels PlutoUI.Slider(2:1:6;show_value=true) )|
...
| | | |
|---|---|---|
|subj | intercept|$(@bind s_int PlutoUI.Slider(0:0.1:5;default=1,show_value=true) )|
|subj | condition|$(@bind s_cond PlutoUI.Slider(0:0.1:5;show_value=true) )|
|item | intercept|$(@bind i_int PlutoUI.Slider(0:0.1:5;default=1,show_value=true) )|
|item | condition |$(@bind i_cond PlutoUI.Slider(0:0.1:5;show_value=true) )|

"""

# ╔═╡ bd3c38f4-1848-467d-8744-63cf5081c5b4
md"""
# Type-1 of Condition-Effect
"""
...
# ╔═╡ 1bb476e2-0eda-4ed5-b1af-faffaca98926


# ╔═╡ a01f6928-0253-417c-8c9c-0428d0a2c0ca
begin
	# generate factorial
	d = factorproduct((;subject=nlevels(nsub, "S")),(;item=nlevels(nitem, "I")),(;condition=nlevels(condlevels,"C")),(;repetition=1:repeats))|>DataFrame
	d.y .= rand(size(d,1)) # dummy response
end;

# ╔═╡ b808036d-b9ad-4203-95ff-48a9dafdffd1
begin
form = @formula(y~1+condition+(1+condition|subject)+(1+condition | item))
...
begin
	...
	mFull = fit(MixedModel,@formula(y~1+condition+zerocorr(1+condition|subject)+zerocorr(1+condition|item)),d)
	mItem = fit(MixedModel,@formula(y~1+condition+(1|subject)+(1|item)),d_sim)
	mSlope = fit(MixedModel,@formula(y~1+condition+zerocorr(1+condition|subject)),d_sim)
...
begin
	function simModels(m0,d_sim,s_int,s_cond,i_int,i_cond;rep=100)
		m0 = deepcopy(m0)
	update!(m0; subject=create_re(s_int,s_cond), item=create_re(i_int,i_cond))
...
function sim_type1(m0,d_sim;s_int=1.,s_cond=0.,i_int=1.,i_cond=0,rep=simulations)
	
 	r = simModels(m0,d_sim,s_int,s_cond,i_int,i_cond;rep=rep)|>
...
let
	...
	d = data(sim_type1(m0,d_sim;s_int=s_int,s_cond=s_cond,i_int=i_int,i_cond=i_cond))
	
	d * mapping(:model=>"",
		:pvalue_function=>"type-1 error %",
		color=:predictor)*
	(visual(Lines)+visual(Scatter))|>x->draw(x;axis=(
	title="($s_int + $s_cond|sub)+($i_int + $i_cond|item)",xticklabelrotation=π/8),palettes=(; color=ColorSchemes.Set1_3.colors))
hlines!([0.05],color=:black,linestyle=:dot)
	ylims!(0,1.)
	current_figure()
end

# ╔═╡ 16a9838c-4da2-496d-9df1-09b2f9a545e7
d_plt = vcat(
	sim_type1(m0,d_sim;s_int=1.,s_cond=0,i_int=0,i_cond=0),
	sim_type1(m0,d_sim;s_int=1.,s_cond=1,i_int=0,i_cond=0),
	sim_type1(m0,d_sim;s_int=1.,s_cond=0,i_int=1,i_cond=0),
	sim_type1(m0,d_sim;s_int=1.,s_cond=1,i_int=1,i_cond=0),
	
	sim_type1(m0,d_sim;s_int=1.,s_cond=0,i_int=0,i_cond=1),
	sim_type1(m0,d_sim;s_int=1.,s_cond=1,i_int=0,i_cond=1),
	sim_type1(m0,d_sim;s_int=1.,s_cond=0,i_int=1,i_cond=1),
	sim_type1(m0,d_sim;s_int=1.,s_cond=1,i_int=1,i_cond=1),
	
);

# ╔═╡ 5e66f281-b3ea-44c4-9454-ec01bd1203a6
let
	d_plt = deepcopy(d_plt)
...
data(d_plt|>
		x->sort(x,:model)|>
		x->subset(x,:predictor=>x ->x.!="(Intercept)")
)*
	mapping(:model=>"",
		:pvalue_function=>"type-1 error %",
		color=:s_cond=>nonnumeric=>"Subj random slope" ,
		marker=:i_int=>nonnumeric =>"Item random Interc.",
		linestyle=:i_int=>nonnumeric=>"Item random Interc.",
		col=:i_cond=>nonnumeric)*
	(visual(Lines)+visual(Scatter))|>x->draw(x;axis=(title="Item random slope\n\n\n\n",xticklabelrotation=π/8))
hlines!([0.05],color=:black,linestyle=:dot)
	current_figure()
end
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "900f298292e88e9c0bcdc32ccb307dae2e554df2"
