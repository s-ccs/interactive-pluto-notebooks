...
# ╔═╡ 22a63f8e-b03a-4fcb-9fda-abd151246093
begin
	using Unfold # analysis
	using UnfoldSim # simulation
	using Random # setting seed
	using CairoMakie # plotting
	using MakieThemes # nicer figure
	using PlutoUI # sliders
	using PlutoUI.ExperimentalLayout: vbox, hbox, Div # pluto layout
	using HypertextLiteral # pluto layout
end

# ╔═╡ 75321386-a344-443e-8d82-98941e886915
html"""
<div style="text-align: center;">
<h2>The effect of estimation time window length on overlap correction in EEG data</h2>

René Skukies & Benedikt V. Ehinger - <a href="https://www.s-ccs.de">www.s-ccs.de</a><br>
...
md"""
### Simulate data
We use UnfoldSim.jl to simulate the data
"""

# ╔═╡ 1fcaaf8d-2e77-4c82-8eef-f8af7cf1b875
md"""
### Modelfit 
Fit the actual model using Unfold
"""

# ╔═╡ 3272a39e-d32f-4ea2-87ed-79a7a47a20c4
md"""
### Figure + plot
create and scale a figure + axis, needs to be redone (WGLMakie + PlutoSliderServer doesnt yet work)
"""

# ╔═╡ cc536fdc-068c-4546-8729-593ccc9745f1
set_theme!(MakieThemes.ggthemr(:fresh))

# ╔═╡ aa86d769-0824-4862-9bfe-b20e82581294
	# in the paper we used 250Hz - but for demo-purposes 100 is plenty
	    sfreq = 100.

# ╔═╡ 17448999-c780-4569-9484-86234876fafa
md"""
## Sidebar
some CSS + JS to put the slider to the side. Thanks to Luis Lips!
"""

# ╔═╡ f1f59430-05d1-49b6-84f8-1ba48be4d406


# ╔═╡ ba1d8cdf-2342-40c7-a89e-6a964a7b446b
# sliders as a table are nice and easy :)
sliders = Div([md"""
|   |   |
|---:|:---:|
|Estimation window| $(@bind ew_time PlutoUI.RangeSlider(-0.5:0.05:2,default=-0.1:0.05:0.6)) |
|noise level | $(@bind noiselevel PlutoUI.Slider(0:2:20,show_value=true,default=5)) |
|noise type|$(@bind noisetype PlutoUI.Select(["PinkNoise"=>"Pink (1/f)","RedNoise"=>"Red (1/f²)","WhiteNoise"=>"White"])) |
|random seed| $(@bind seed PlutoUI.Slider(1000:1100,show_value=true))|
"""
]);
...
begin
	...
	function gen_data(rng,noiselevel,noise)
		...
		if noise == "PinkNoise"
			f= PinkNoise
		elseif noise == "RedNoise"
			f = RedNoise
		elseif noise == "WhiteNoise"
			f = WhiteNoise
		else
			@show noise
			error("wrong noise")
		...
		dat,evts = UnfoldSim.predef_eeg(rng;sfreq=sfreq,
			...
			n_repeats=20,noise=f(;noiselevel=noiselevel),onset=UniformOnset(;offset=sfreq*0.2,width=sfreq*0.3))
		return (;dat,evts)
	end

obs_data  = gen_data(seed,noiselevel,noisetype);
end;
...
|Uniform overlap| 0.2s - 0.5s|
"""

# ╔═╡ fe11d528-d797-4a48-aa45-f1c96dc2def7
begin
function calc_model(τ,(;evts,dat))

	m = fit(UnfoldModel,
		Dict( Any=>(@formula(0~1), firbasis(τ,sfreq,string(τ)))
		),evts,dat);
	c = coeftable(m)
	time = c.time
	estimate = c.estimate
...
result = calc_model((ew_time[1],ew_time[end]),obs_data);
end;

# ╔═╡ 0074f394-1fdd-4e09-b15a-601e35c50674
begin
	#calculate ground truth
	gt_result = calc_model((0,0.5),gen_data(1,0,"WhiteNoise"));
end;

# ╔═╡ 65582beb-d909-4251-8215-16ca2acc7acd
begin
	
	f = Figure()
	ax = f[1,1] = Axis(f,xlabel="time [s]",ylabel="voltage [µV]")
	lines!(ax,gt_result.time,gt_result.estimate,color=:gray);
	lines!(ax,result.time,result.estimate);
...
manifest_format = "2.0"
project_hash = "3ffd75ad341e038c877b02481ff2a1806ba5f078"
...
git-tree-sha1 = "551fd7902ebdac5e346500d9a7a6afb3f3502c83"
...
git-tree-sha1 = "355fa41bdded081a2ad8cf13d21c3fd4d7a164f9"
