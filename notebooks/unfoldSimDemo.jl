...
# ╔═╡ 3d64dad1-617e-41a4-9429-029e32ceaa54
begin
...
# ╔═╡ fccba0a1-c0ef-4a83-a501-f8ae33b05825
md"""
# UnfoldSim Power Demo
Move the sliders below to change noise and number of trials. Observe the power!
"""

# ╔═╡ b9cbfec6-2627-4cea-9a1c-6f42551fb66c
md"""
|  |  |
|---|---|
|noiselevel|$(@bind noiselevel PlutoUI.Slider(0:1:10,default=1,show_value=true))|
|trials|$(@bind ntrials PlutoUI.Slider(5:5:50,default=20,show_value=true))|
|||
"""

# ╔═╡ 6f6c7347-735c-4d3b-b104-d0d77c34ddd6
md"""
...
# ╔═╡ 52a42ee1-c696-4956-9491-e40e0fc5248e
# function that simulates data once
function sim_once(seed)
	data,evts = UnfoldSim.predef_eeg(MersenneTwister(seed);
		n_repeats=ntrials,
		return_epoched=true,
		sfreq=200,
		p3 = (p300(;sfreq=200), @formula(0~1),[5],Dict()),
		noise=PinkNoise(noiselevel=noiselevel))
    return data,evts
end;

# ╔═╡ 8889d53a-878e-4ebe-886f-a36ecb5dbb70
begin
data,evts=sim_once(1)
	f = Figure()
	ax = f[1,1] = Axis(f)
	vspan!(ax,26,36,color=("gray",0.4))
	series!(ax,data[:,evts.condition.=="face"]',solid_color=("dodgerblue1",0.05))
	series!(ax,data[:,evts.condition.=="car"]',solid_color=("maroon4",0.05))

	lines!(ax,mean(data[:,evts.condition.=="face"][:,1],dims=2),color="dodgerblue1",linewidth=5)
	lines!(ax,mean(data[:,evts.condition.=="car"][:,1],dims=2),color="maroon4",linewidth=5)
	
	f
end

# ╔═╡ 69d83d97-0cc6-4bf7-a12f-d1adf1c8c45d
begin
...
for seed = eachindex(pvals)
	# sim data
    data,evts = sim_once(seed)

	# extract time range to test
    evts.y = dropdims(mean(data[26:36,:],dims=1),dims=(1))
	face =  @subset(evts,:condition.=="face").y
	car =   @subset(evts,:condition.=="car").y
    ## calculate a two-sided t-test
    pvals[seed] = pvalue(UnequalVarianceTTest(face,car))
...
# ╔═╡ 98460a34-f53e-4904-b80f-ce8a6d4b0965
md"""
The power of this simulation is approximately $power %
"""

# ╔═╡ 2eda9bb1-9bca-4eda-b19c-bae9e5cc9ec0
set_theme!(theme_ggthemr(:fresh))

# ╔═╡ 94387f48-0f3a-4b48-acbb-9e4657087010
...
manifest_format = "2.0"
project_hash = "4853912038e0909644d5217164bfe9c07b34293b"
...
git-tree-sha1 = "e445a26217944cc1597e2879c604bd6b47bd7c78"
...
git-tree-sha1 = "62bb438de5c3d95774dfc89eabcb434d2ee1da70"
